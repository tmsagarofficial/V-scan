...
def generate_report():
    try:
        ...
        for r in scan_results:
            ...
            formatted_results.append(f"Test: {r.get('test', '')}\nResult: {r.get('result', '')}\nSeverity: {severity_level}\n")
        ...
        safe_results = [r for r in scan_results if r.get("severity") == "safe"]
        ...
        try:
            ...
            if hasattr(response, 'text'):
                ...
                # Fallback in case the Gemini API response structure is different
                report = str(response)
...
def calculate_cvss_score(critical_issues, warning_issues, cvss_version):
    """Calculate a simplified CVSS score based on the findings"""
    ...
    if not critical_issues and not warning_issues:
        return {"score": 0.0, "severity": "None", "vector": "CVSS:3.1/AV:N/AC:H/PR:H/UI:R/S:U/C:N/I:N/A:N"}
    
    # Count various types of issues
    headers_missing = sum(1 for r in critical_issues if 'Missing' in r.get('result', '') and any(h in r.get('test', '') for h in ['CSP', 'HSTS', 'X-Content-Type-Options', 'X-XSS-Protection']))
    exposed_files = sum(1 for r in critical_issues if '.git' in r.get('test', '') or '.env' in r.get('test', ''))
    https_issues = sum(1 for r in critical_issues if 'HTTPS' in r.get('test', ''))
    
    # Calculate base score - simplified approach
    # In a real implementation, you'd use the full CVSS calculator
    if cvss_version == "CVSS v3.1":
        # For CVSS v3.1
        if exposed_files > 0:
            base_score = 8.6  # Critical
            severity = "Critical"
        ...
        elif headers_missing > 2:
            base_score = 6.5  # Medium
            severity = "Medium"
            vector = "CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:U/C:L/I:L/A:L"
        elif headers_missing > 0 or len(warning_issues) > 2:
            base_score = 4.3  # Medium
            severity = "Medium"
    ...
    # Determine severity if not yet set
    if not severity:
        if base_score >= 9.0:
            severity = "Critical"
        elif base_score >= 7.0:
    ...
    return {"score": base_score, "severity": severity, "vector": vector}
